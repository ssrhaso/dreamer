...
def load_model(checkpoint_path: str, device: str = 'cpu'):
    ...
    model = HRVQTokenizer(
        ...
        commitment_costs=[0.05, 0.25, 0.60],  # Must match training config
    ).to(device)
...
def main(checkpoint_path='checkpoints/vq_model_best.pth'):
...
if __name__ == '__main__':
    import sys
    checkpoint = sys.argv[1] if len(sys.argv) > 1 else 'checkpoints/vq_model_best.pth'
    print(f"\n Loading checkpoint: {checkpoint}\n")
    main(checkpoint_path=checkpoint)
